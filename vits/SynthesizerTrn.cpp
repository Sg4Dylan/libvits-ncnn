...
bool SynthesizerTrn::load_model(const std::string &folder, Net &net, const string &name, bool multi,
    ...
    bool param_success = !net.load_param(param_path.c_str());
    bool bin_success = !net.load_model(bin_path.c_str());
    if (param_success && bin_success) {
        // LOGI("%s loaded!", name.c_str());
        return true;
    }
    if (!param_success) cout << "param load fail" << endl;
    if (!bin_success) cout << "bin load fail" << endl;
    return false;
}
...
SynthesizerTrn::~SynthesizerTrn() {
    cout << "clearing nets..." << endl;
    clear_nets();
    cout << "nets cleared!" << endl;
}
